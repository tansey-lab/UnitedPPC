...
from Performance_Benchmarking.scripts.pyro_model import PlackettLuce_2D
import torch
...
def summarize_cv_results(cv_dims, cv_step, n_folds, results_dir, plots_dir):
    ...
    print(f'Best dims is {n_dims_min}, while one se rule dims is {n_dims_one_se_rule}, the elbow dims is {n_dims_knee}')
...
def summarize_cv_results_mae(cv_dims, cv_step, n_folds, results_dir, plots_dir):
    n_dims_options = np.arange(start=cv_dims[0], stop=cv_dims[1], step=cv_step)
    scores = np.zeros((len(n_dims_options), n_folds))
    with open(f"{results_dir}/scores_mae.txt", "r") as f:
        for line in f:
            sp = line.split()
            scores[int(sp[0]), int(sp[1])] = float(sp[2])  # mae
            #scores[int(sp[0]), int(sp[1])] = float(sp[2].replace('tensor(', '').rstrip(','))
    ...
    n_dims_knee = kneedle.knee
    knee_y = kneedle.knee_y
    print(f'Best dims is {n_dims_min}, while one se rule dims is {n_dims_one_se_rule}, elbow is {n_dims_knee}')
    plot_cv_scores_mae(scores_means, scores_se, n_dims_options, cutoff, n_dims_min, n_dims_one_se_rule,
                   n_dims_knee, knee_y, results_dir, plots_dir)
...
# plot the cv_score
def plot_cv_scores_mae(scores_means, scores_se, n_dims_options, cutoff, n_dims_min, n_dims_one_se_rule,
                   n_dims_knee, knee_y, results_dir, plots_dir):
    plt.rcParams['figure.figsize'] = [6, 6]
    plt.plot(n_dims_options, scores_means, color='black')
    plt.fill_between(n_dims_options, scores_means + 1 * scores_se, scores_means - 1 * scores_se, alpha=.1)
    ...
    plt.title(results_dir.rsplit("/", 1)[-1])  # add title
    plt.grid()
    plt.savefig(f'{plots_dir}/cv_score_mae.pdf')
    plt.close()
